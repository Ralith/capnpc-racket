...
(: int-list-ref (-> Byte Boolean (ListReference Integer) Index Integer))
(define (int-list-ref exponent signed xs i)
  (assert (eq? 'data (ListReference-element-type xs)))
  ((make-int-accessor exponent signed i) xs))
  ...
  (if (= 0 exponent)
      (lambda ((ref : reference))
        (let ((loc (reference-location ref)))
          (let ((byte (bytes-ref (vector-ref (reference-message ref) (location-segment loc))
                                 (+ (location-byte loc) offset))))
            (if signed
                (- (bitwise-xor byte #x80) #x80)
                byte))))
